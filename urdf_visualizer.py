...
class URDFVisualizer:
    def __init__(self, args, graph_initial, graph_final, graph_predicted, 
                 contact_node_gt, contact_force_gt, contact_node=None, contact_force=None):
        self.args = args
    ...
    def create_env(self):
        ...
        dof_props['stiffness'][:3].fill(10000000)
        dof_props['damping'][:3].fill(10000)
    ...
    def visualize(self):
        frame = 0
        rb_force_tensor = torch.zeros((self.num_envs, self.num_rigid_bodies, 3), device='cpu', dtype=torch.float)
        ...
        while not self.gym.query_viewer_has_closed(self.viewer):
            ...
            for env_id in range(self.num_envs):
                ...
                if self.contact_node is not None: # If we are visualizing control policy predictions            
                    ...
                    if frame>50:
                        rb_force_tensor[env_id, self.asset_rb_handles_dict['predicted'][contact_node], :] = contact_force.detach().cpu()

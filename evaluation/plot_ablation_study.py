...
if __name__ == '__main__':
    ...
    width = 0.15
    for i, data_file in enumerate(cfg.ablation_study_data_files):
        with open(data_file, 'rb') as f:
            data = pickle.load(f)
        ind = data['num_nodes']
        if i==0:
            bar = ax.bar(ind, np.mean(data['max_distance_displacemnts_list'], axis=1), width=0.2, color=tab20c[i])      
            for idx in trained_systems:  
                bar[idx].set_color(tab20c_dark[i])
        bar = ax.bar(ind+width*(i+1), np.mean(data['max_distance_errors_list'], axis=1), width=0.2, color=tab20c[i+1])
        for idx in trained_systems:  
    ...
    for i, data_file in enumerate(cfg.ablation_study_data_files):
        policy = data_file.split('-')[0].split('_')[2:]
        policy = ' '.join(policy)
    ...
    # Instead of plotting each bar for node size, plot a single bar with the mean and std
    bar_graph_dict = {}
    for i, data_file in enumerate(cfg.ablation_study_data_files):
        with open(data_file, 'rb') as f:
            data = pickle.load(f)
        policy = data_file.split('-')[1]
        #policy = ' '.join(policy)        
        #if i==0:
        #    max_distance_mean = np.mean(np.array(data['max_distance_displacemnts_list']).flatten())
        #    max_distance_std = np.std(np.array(data['max_distance_displacemnts_list']).flatten())
        #    bar_graph_dict['no action'] = {'mean': max_distance_mean, 'std': max_distance_std}
        max_error_mean = np.mean(np.array(data['max_distance_errors_list']).flatten())
        max_error_std = np.std(np.array(data['max_distance_errors_list']).flatten())
        bar_graph_dict[policy] = {'mean': max_error_mean, 'std': max_error_std}
    fig, ax = plt.subplots(figsize=fig_size)
    #ax.bar(bar_graph_dict.keys(), [x['mean'] for x in bar_graph_dict.values()], yerr=[x['std'] for x in bar_graph_dict.values()], align='center', alpha=0.5, ecolor='black', capsize=10)
    ax.bar(bar_graph_dict.keys(), [x['mean'] for x in bar_graph_dict.values()], align='center', alpha=0.5, ecolor='black', capsize=10)
    ax.set_ylabel('Distance (m)')
    ax.yaxis.label.set_fontweight('bold')
    #ax.set_ylim(cfg.y_axis_range[0], 0.2)
    ...
    for i, bar in enumerate(ax.patches):
        if i<4:
            bar.set_color(tab20c_dark[0])
        elif i>4:
            bar.set_color(tab20c_dark[1])
        else:
            bar.set_color(tab20c_dark[4])

    plt.show()

    print([x['mean'] for x in bar_graph_dict.values()])
